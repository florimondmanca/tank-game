...
import pygame
import os
from src.walls import Wall, Pit
...
def get_type(object_type, voisins):
    """Get the type of an object according to its neighborhood."""
...
# n est le numéro du niveau à charger. -1 pour le menu. Si custom est
# True, on va chercher un niveau custom
def get_level(path, n, custom=False):
    """Get the level."""
    ...
    if n == -1:
        ...
        if custom:
            level_path = join(path, join("custom_levels",
                                         "custom_level{}.txt".format(n)))
        else:
    ...
    with open(level_path, mode="r") as texte:
        ...
        for i in range(21):
            for j in range(32):
                ...
                var = map_niveau[i][j]
                if var == 1:
                    typemur = get_type("wall",
                                       get_voisins(map_niveau, i, j, 1))
                    walls_group.add(Wall(path, typemur, i, j))
                elif var == 2:
                    typemur = get_type("pit",
                                       get_voisins(map_niveau, i, j, 2))
                    ...
                    if var == 3:
                        pos_player = pos
                    if var == 4:
                        pos_AI.append(["yellow", pos])
                    elif var == 5:
                        pos_AI.append(["yellowPlus", pos])
                    elif var == 6:
                        pos_AI.append(["blue", pos])
                    elif var == 7:
                        pos_AI.append(["bluePlus", pos])
                    elif var == 8:
                        pos_AI.append(["red", pos])
                    elif var == 9:
                        pos_AI.append(["redPlus", pos])
                    elif var == 10:
                        pos_AI.append(["purple", pos])
                    elif var == 11:
                        pos_AI.append(["purplePlus", pos])
                    elif var == 12:
                        pos_AI.append(["spawner", pos])
                    elif var == 13:
                        pos_AI.append(["spawnerPlus", pos])

        # on obtient ensuite la liste des déplacements des IA bleues

        n_blues = len([IA for IA in pos_AI
                       if IA[0] == "blue" or IA[0] == "bluePlus" or
                       IA[0] == "purple" or IA[0] == "purplePlus"])

        liste_dep = texte.readline().strip().split()
        # list_dep contient la liste non-organisée en tuples des coordonnées
        # des points à atteindre : x1 y1 x2 y2 x3 y3 etc.

        points_list = []  # contiendra l'ensemble des traj des IA du niveau

        for k in range(n_blues):  # d"abord les IA bleues
            liste = []
            for i in range(len(liste_dep) // 2):
                liste.append((32 * int(liste_dep[0]) + 16,
                              32 * int(liste_dep[1]) + 16))
                liste_dep.pop(0)
                liste_dep.pop(0)
...
def init_unlocked(a_bool):
    """Initialize the unlocked text file."""
...
def get_unlocked():
    ...
    # liste[i] permet de savoir si le niveau i est débloqué ou non.
    # Liste[0] est inutile


def overwrite_unlocked(liste):
    """Overwrite the unlocked file with a given list of unlocked levels."""
    unlock_path = join(path, "unlocked.txt")
    with open(unlock_path, mode="w") as texte:
